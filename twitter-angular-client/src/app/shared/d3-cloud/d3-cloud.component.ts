...
@Component({
  selector   : 'app-d3-cloud',
  template   : `<div class='word-cloud'></div>`
})

// The D3 Cloud Component.
// TODO: Currently tightly coupled with word cloud requirements.
// Make the option to take other features as part of options/settings.
export class D3CloudComponent implements OnInit, OnChanges {
  ...
  ngOnChanges(changes : SimpleChanges) {
    if (changes.wordData.currentValue) {
      const cls = this;
      this.initialize(cls);
    }
  }
  ...
  private initialize(cls) {
    this.data = this.wordData.data.map(function(d) {
      return {
        text: d.name, volume: d.volume != 0 ? d.volume : 0 , size: cls.getRandom()
      }
  ...
  private buildSVG() {
    ...
    this.tooltip = d3.select('div.word-cloud').append("div")
